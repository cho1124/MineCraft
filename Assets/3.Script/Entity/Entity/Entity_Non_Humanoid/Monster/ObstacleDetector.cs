...
public class ObstacleDetector : MonoBehaviour
{
    ...
    private Entity_Non_Humanoid entity_non_humanoid;
    ...
    private void Start()
    {
        entity_non_humanoid = GetComponentInParent<Entity_Non_Humanoid>();
        if (entity_non_humanoid == null)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (entity_non_humanoid != null) {
                //entity_non_humanoid.JumpAndChangeState();
            }
        }
    }
    ...
    private IEnumerator ChaseTarget(Transform target, bool isPlayer)
    {
        ...
        //entity_non_humanoid.IsChasingPlayer = isPlayer;
                ...
                if (Time.time < raycastEndTime)
                {
                    ...
                    if (Physics.Raycast(transform.position, direction, out hit))
                    {
                       // Debug.DrawRay(transform.position, direction * hit.distance, Color.red);
                        ...
                        //if (hit.transform.CompareTag(playerTag) || (hit.transform.CompareTag(animalTag) && !entity_non_humanoid.IsChasingPlayer))
                        //{
                        //    //entity_non_humanoid.SetTarget(target.position);
                        //}
                    }
                }
            }
        ...
        Debug.Log("추격을 그만둡니다. ");
        //entity_non_humanoid.EndChaseAndWander();
    }
}
